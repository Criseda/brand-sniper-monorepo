...
- `LLM_BASE_URL` / `LLM_MODEL` / `LLM_API_KEY` — any OpenAI-compatible Chat Completions endpoint whose model supports function/tool calling (e.g. Groq at `https://api.groq.com/openai/v1` with `qwen/qwen3.8-27b` — get a free key at [console.groq.com/keys](https://console.groq.com/keys))
