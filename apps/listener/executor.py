...
class ExecutionService(abc.ABC):
    ...
    """
    @abc.abstractmethod
    async def execute(self, market_hash_name: str, purchase_price_cents: int, estimated_profit_cents: int, z_score: float) -> None:
        pass
...
class PaperExecutor(ExecutionService):
    ...
    async def execute(self, market_hash_name: str, purchase_price_cents: int, estimated_profit_cents: int, z_score: float) -> None:
        ...
        print(f"[PAPER TRADE] Simulated Buy | Item: {market_hash_name} | Price: ${purchase_price_cents/100:.2f} | Est. Profit: ${estimated_profit_cents/100:.2f} | Z-Score: {z_score:.2f}")
